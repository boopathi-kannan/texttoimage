  ...
  return (
    <div className='fixed top-0 left-0 right-0 bottom-0 z-10 backdrop-blur-sm bg-black/30 flex justify-center items-center'>
      <motion.form onSubmit={onSubmitHandler}  className='relative bg-white p-10 rounded-xl text-slate-500' 
       initial={{opcity:0.2,y:50}}
    transition={{duration:0.3}} whileInView={{opacity:1,y:0}} viewport={{once:true}}>
        <h1 className='text-center text-2xl text-neutral-700 font-medium'>{state}</h1>
        <p className='text-sm'>Welcome back! Please sign in to continue</p>
        {
          state!=='Login' && 
          <div className='px-6 py-2 flex item-center gap-2 rounded-full mt-4'>
         ...
         <div className='px-6 py-2 flex item-center gap-2 rounded-full mt-4'>
          ...
          value={password} type='password' placeholder='Password' className='outline-none text-sm' required/>
        </div>
